...
function [mxr,bnvalues] =DEsc6(num,yn,xr,nsd)

NOBJ = 1;                          % Number of objectives
MODEDat.NRES = 0;                          % Number of constraints
Nvar   = 4;                       % Numero of decision variables
MODEDat.mop = str2func('fitnessfuncsc6');    % Cost function

MODEDat.FieldD =[0 1000;0 1;.1 20;.1 4]; % Initialization bounds
MODEDat.Initial=[0 1000;0 1;.1 20;.1 4]; % Optimization bounds
nsd;
...
Generaciones  = 5; %MODEDat.MAXGEN;    % Maximum number of generations.
...
sof = size(Parent);
for i=1:sof(1)
    JxParent(i,1) = fitnessfuncsc6(Parent(i,:),xr,yn);
end
FES = FES+Xpop;   
...
for n=1:Generaciones 
    ...
    sofc = size(Child);
for i=1:sofc(1)
    JxChild(i,1) = fitnessfuncsc6(Child(i,:),xr,yn);
end
