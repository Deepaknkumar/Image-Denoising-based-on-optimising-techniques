...
%           minor revisions in order to solve the 25 benchmark test functions,
    ...
    function [mxr,bnvalues] = JADEsc6(popsize,yn,xr,nsd)
   ...
   iter = nsd;
    ...
    %popsize = 10;
     lu = [0,0,.1,.1;1000,1,20,4];
        ...
        for i=1:popsize
        valParents(i,:) = fitnessfuncsc6(popold(i,:),xr,yn);
        end
        ...
        while FES  < n * 5 %& min(fit)>error_value(problem)
            ...
            for inb=1:popsize
                valOffspring(inb,:) = fitnessfuncsc6(popold(inb,:),xr,yn);
            end
            FES = FES + popsize;
