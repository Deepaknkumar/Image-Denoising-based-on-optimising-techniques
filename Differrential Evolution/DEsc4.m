...
function [mxr,bnvalues] =DEsc4(num,yn,xr,nsd)
...
Generaciones  = 40; %MODEDat.MAXGEN;    % Maximum number of generations.
...
sof = size(Parent);
for i=1:sof(1)
    JxParent(i,1) = fitnessfuncsc4(Parent(i,:),xr,yn);
end
FES = FES+Xpop;   
...
for n=1:Generaciones 
    ...
    sofc = size(Child);
for i=1:sofc(1)
    JxChild(i,1) = fitnessfuncsc4(Child(i,:),xr,yn);
end
